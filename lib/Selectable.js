...
class Selectable {
        ...
        document.querySelectorAll('.selectable-option').forEach(function(element) {
            element.addEventListener('click', function() {
                ...
                if (selectedOption) {
                    ...
                    that._selectOptionCountLabel(selectField, wrapperDiv);
                    return true;
                }
                if (! optionWithValue) {
     ...
     * @private
     */
    _selectOptionCountLabel(selectField, wrapperDiv) {
        let selectedOptions = selectField.querySelectorAll('option[selected="selected"]');
        if (selectedOptions.length > 0) {
            let optionsLabel = 'Elements checked';
            if (selectField.getAttribute('data-option-count')) {
                optionsLabel = selectField.getAttribute('data-option-count');
            }
            wrapperDiv.querySelector('.selectable-title').innerHTML = selectedOptions.length + ' ' + optionsLabel
        } else {
